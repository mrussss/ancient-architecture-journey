...
- 四张 AI 生成正式背景图，Python 标准库脚本只作为 fallback 占位素材生成器
- `--smoke-test` 模式
...
这些背景优先由游戏加载并缩放到 960x540。`tools/generate_assets.py` 只用于生成缺失资源和低质量 fallback；脚本会保留已经存在的背景图，不会覆盖正式背景。
...
2. Level 2 Huizhou：屋檐 / 木梁跳跃关，先上屋檐再回到庭院，地面和屋顶各一个木偶。
